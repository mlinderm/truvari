...
def edit_header(my_vcf):
    ...
    header.add_line(('##INFO=<ID=MatchGT,Number=1,Type=String,'
                     'Description="TRUE if call GT matches base GT">'))
    return header
...
def match_calls(base_entry, comp_entry, astart, aend, sizeA, sizeB, regions, reference, args, outputs):
    ...
    if args.gtcomp and not truvari.entry_gt_comp(base_entry, comp_entry, outputs["sampleBase"], outputs["sampleComp"])[0]:
...
def output_base_match(base_entry, num_neighbors, thresh_neighbors, myid, matched_calls, outputs):
    ...
    if not matched_calls[b_key]:
        ...
        match, _ = truvari.entry_gt_comp(base_entry, thresh_neighbors[0].match_entry, outputs["sampleBase"], outputs["sampleComp"])
        if match:
...
def report_best_match(base_entry, num_neighbors, thresh_neighbors, myid, matched_calls, outputs, args):
    ...
    for neigh in thresh_neighbors:
        ...
        if not matched_calls[c_key]: # unmatched
            ...
            match, gts = truvari.entry_gt_comp(base_entry, neigh.match_entry, outputs["sampleBase"], outputs["sampleComp"])
            if match:
                ...
                gt_match = False
            outputs["stats_box"]["TP-gt_call-{1}_base-{0}".format(*gts)] += 1
        ...
        match_entry.info["MatchGT"] = "TRUE" if gt_match else "FALSE"
