...
def entry_variant_type(entry):
    ...
    """
    # Allow for subtypes in the representation
    ...
    if "SVTYPE" in entry.info:
        ...
        if type(ret_type) is list:
            ...
            ret_type = ret_type[0]
        colon_pos = ret_type.find(":")
        if colon_pos != -1:
            logging.warning("SVTYPE has subtypes for entry %s, using top-level type", str(entry))
            return ret_type[:colon_pos]
        return ret_type
